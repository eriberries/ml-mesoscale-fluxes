...
# M = 5205297 # size of dataset, here:training set
n = 997716 # size of the test set

pred_reps = 9 # the number of prediction repetition, the divisor of the datasetsamplesize by the testsetsamplesize plus the remainder

# cat = "mainmodel" ## this is the line to be specified when running
...
for cat in cat_keys:
    ...
    def Get_datasets(I = 0,
                     fn = [f"NN_data_train_input_normalized_level_Q_discard10_withExtremes_noMemory_X5_final.npy",
                           f"NN_data_train_output_normalized_momentum_Q_discard10_withExtremes_noMemory_X5_final.npy"]):
        dataset_list = []
    ...
    predicted_list = []
    for i in range(pred_reps):
        dataset = Get_datasets(I=i)[0]
    ...
    np.save(datadir+ f"predicted_trainingset_{cat}.npy", np.concatenate(predicted_list, axis=0))
    print(datadir+ f"predicted_trainingset_{cat}.npy")
